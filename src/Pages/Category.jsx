...
export default function Category() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API}/api/v1/customer/product-category?page=1&limit=15`);
    ...
    <div className="w-full overflow-x-auto scrollbar-hide font-dm">
      <div className="flex md:space-x-8 space-x-4 py-3 px-4">
        {categories.map((category) => (
          <button
            key={category._id}
            className="flex-shrink-0 px-4 bg-gray-200 text-[#004F44] rounded-lg hover:bg-gray-300 transition duration-200
            text-sm md:text-lg font-light md:px-8 py-1 w-auto"
          >
